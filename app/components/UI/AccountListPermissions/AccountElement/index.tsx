...
const AccountElement = ({
  ...
  let isActive = false;
  if (
    (index === 1 && isConnected && !selectedIsConnected) ||
    (isSelected && isConnected)
  ) {
    isActive = true;
  }

  const selectedIsNotConnected =
    !isConnectRequest && isSelected && !isConnected;
  ...
  return (
    <View onStartShouldSetResponder={() => true}>
      <View
        style={[
          styles.account,
          isLastConnected || (isSelected && !isConnected && !isConnectRequest)
            ? styles.border
            : null,
        ]}
        key={`account-${address}`}
      >
        <View
          style={[styles.row, selectedIsNotConnected ? styles.selected : null]}
        >
                ...
                <Text numberOfLines={1} style={[styles.accountLabel]}>
                  ...
                  <Text style={styles.notConnectedText}>
                    {selectedIsNotConnected
                      ? ` (${strings(
                          'account_approval_permissions.account_not_connected',
                        )})`
                      : ''}
                  </Text>
                </Text>
